//Server for Every Man for himself game. Created by RPI Webscience group 6 Spring 2017.
...
io.sockets.on('connection', function(socket){
  ...
  function playerEnter(data) {
    ...
    if( lobby != undefined ){
      ...
      //check trapper first
      if(globalGameInfo[data["lobbyID"]]["trapper"] == data["user_name"]){
        //Send error to user to pick new name
        io.to(socket.id).emit('duplicateNameError', { "message": "The name you picked is already used in the lobby. Please pick a new name." } );
        return;
      }

      //check in prisoner list
      for(var i = 0; i < globalGameInfo[data["lobbyID"]]["prisoners"].length; i++ ){

        //if a duplicate in the lobby
        if(globalGameInfo[data["lobbyID"]]["prisoners"][i]["name"] == data["user_name"]){
          //Send error to user to pick new name
          io.to(socket.id).emit('duplicateNameError', { "message": "The name you picked is already used in the lobby. Please pick a new name." } );
          return;
        }
  ...
  //fires when a prisoner chooses a door
  ...
  function endTurn(lobbyID){
    ...
    //loop through all prisoners and check who is dead
    var killedList = [];
    var allDead = true;
    for(var i = 0; i < globalGameInfo[lobbyID]["prisoners"].length; i++){
    ...
    if(globalGameInfo[lobbyID]["currentRound"] >= globalGameInfo[lobbyID]["maxRounds"] || allDead){
      ...
      //in this case the game is over and send out end conditions
      io.sockets.in(lobbyID).emit('endGame', endGameMsg);

    }
    else{
      //in this case, tell all players in lobby to move to next turn
  ...
  function refreshLeaderboards(lobbyID){
    ...
    killerboard.find({}, function(err, killers) {
      ...
      playerboard.find({}, function(err, prisoners){
        ...
        //ssend out the leaderboard info to clients
